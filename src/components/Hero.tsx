...
export default function Hero() {
  ...
  return (
    <section id="home" className="relative bg-white overflow-hidden border-b border-[#e5e7eb] py-10 lg:py-16">
      ...
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-gradient-to-bl from-sky-100/60 via-cyan-50/40 to-transparent rounded-full blur-3xl pointer-events-none -z-10" />
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 items-center">
        ...
        <div className="lg:col-span-7 z-10">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            <span className="inline-flex items-center px-3.5 py-1 rounded-full bg-sky-50 text-[#29b6f6] text-[11px] font-extrabold uppercase tracking-widest border border-sky-100">
              October 21, 2026
            </span>
          ...
          <h1 className="text-[46px] sm:text-[62px] lg:text-[74px] font-black tracking-tight text-[#0f294a] leading-none mb-1">
            Teenspace <span className="text-[#29b6f6]">2026</span>
          </h1>
          ...
          {/* Date Badges Row (Light Blue Theme) */}
          <div className="flex items-center gap-3 mb-6">
            {dates.map((d, i) => (
              <div
                key={i}
                className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-gradient-to-br from-[#29b6f6] to-[#0284c7] text-white font-black text-2xl sm:text-3xl flex items-center justify-center shadow-lg shadow-sky-400/30 tracking-tight transform hover:scale-105 transition-transform"
              >
                {d}
              </div>
            ))}
          ...
          {/* Countdown Grid (4 Cards with Light Blue Accent Bar) */}
          <div className="grid grid-cols-4 gap-3 sm:gap-4 max-w-xl mb-4">
            {countdownBlocks.map((b, i) => (
              <div
                key={i}
                className="bg-white/95 backdrop-blur-md rounded-2xl p-3 sm:p-4 border border-slate-200/90 shadow-lg shadow-slate-200/60 text-center relative overflow-hidden group hover:border-[#29b6f6] transition-all"
              >
                {/* Light Blue accent line on top */}
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-8 sm:w-10 h-[3px] bg-[#29b6f6] rounded-full" />
          ...
          {/* Pill Register Button (Light Blue Theme) */}
          <div>
            <a
              href="#register"
